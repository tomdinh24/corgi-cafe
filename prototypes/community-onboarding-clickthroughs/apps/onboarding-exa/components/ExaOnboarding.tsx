...
export function ExaOnboarding() {
  ...
  const openAccount = () => { setAccountMenuOpen(false); setError(""); setAccountNotice(""); setDeleteArmed(false); router.push("/account"); };
  ...
  const saveFeedback = (rating: "very_unhelpful" | "unhelpful" | "neutral" | "helpful" | "very_helpful") => {
    if (!recommendationId) return;
  ...
  const uploadRecognitionPhoto = async (kind: "self" | "nearby", file: File | undefined) => {
    ...
    try {
      ...
      if (kind === "self") setPhotoSelf(true); else setPhotoNearby(true);
  ...
  const closeAccount = () => { setDeleteArmed(false); setAccountNotice(""); router.back(); };
