...
export function ExaOnboarding() {
  ...
  if (step === 4) return <Shell step={4} wide><Header eyebrow="Your profile" title={<>Add your public links <span className="optional-badge">Optional</span></>}>Share any links that help your profile feel more like you.</Header>{notice && <p className="preview-note">{notice}</p>}<div className="journey-stack"><Field label="LinkedIn" type="url" value={links.linkedin_identifier} onChange={(e) => setLinks({ ...links, linkedin_identifier: e.target.value })} />{linkErrors.linkedin_identifier && <p className="journey-error">{linkErrors.linkedin_identifier}</p>}<Field label="Personal or company website" type="url" value={links.website} onChange={(e) => setLinks({ ...links, website: e.target.value })} />{linkErrors.website && <p className="journey-error">{linkErrors.website}</p>}<Field label="GitHub" type="url" value={links.github} onChange={(e) => setLinks({ ...links, github: e.target.value })} />{linkErrors.github && <p className="journey-error">{linkErrors.github}</p>}<Field label="Other public link" type="url" value={links.social} onChange={(e) => setLinks({ ...links, social: e.target.value })} />{linkErrors.social && <p className="journey-error">{linkErrors.social}</p>}</div><Actions><Primary onClick={() => go(5)} disabled={hasLinkErrors}>Continue</Primary><Quiet onClick={back}>Back</Quiet></Actions></Shell>;
  ...
  if (step === 15) return <Shell step={15}><div className="center">{meetingBothMet ? <><div className="terminal-mark">✓</div><Header eyebrow="Both confirmed" title={`You met ${person.firstName}`}/><div className="approved-links">{postLinks.length > 0 ? postLinks.map((link, index) => link.url.startsWith("http") ? <a key={index} className="approved-link" href={link.url} target="_blank" rel="noopener noreferrer">{socialLabel(link.kind)}</a> : <span key={index} className="approved-link">{socialLabel(link.kind)}: {link.host || link.url}</span>) : <span className="pending-links">{person.firstName} did not add links to share.</span>}</div><p className="helper-text">Only links {person.firstName} approved for after an in-person meeting.</p></> : <><div className="loader"><i/><i/><i/></div><Header eyebrow={`Waiting for ${person.firstName}`} title={`You said you met ${person.firstName}`}>The meeting is confirmed once {person.firstName} also taps "Yes, we met". If they don't, double-check you found the right person. Their links stay private until you both confirm.</Header></>}<Actions><Primary onClick={() => go(16)}>Continue</Primary></Actions></div></Shell>;
